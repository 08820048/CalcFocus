...
import type {
	...
	FrameInstance,
	CalcFocusExtensionAPI,
	CalcFocusExtensionModule,
	RenderHookContext,
	RenderHookFn,
	RenderHookPhase,
} from "./types";
...
const EXTENSION_SETTINGS_STORAGE_KEY = "calcfocus.extension-settings.v1";
...
interface ActiveExtension {
	info: ExtensionInfo;
	module: CalcFocusExtensionModule;
	disposables: (() => void)[];
}
...
export class ExtensionHost {
	...
	async activateExtension(info: ExtensionInfo, moduleUrl: string): Promise<void> {
		...
		let mod: CalcFocusExtensionModule | null = null;
		try {
			...
			try {
				const loaded: CalcFocusExtensionModule = await import(/* @vite-ignore */ moduleUrl);
				mod = loaded;
