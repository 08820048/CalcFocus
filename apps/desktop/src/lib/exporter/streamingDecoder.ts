...
export class StreamingVideoDecoder {
	...
	private toLocalFilePath(resourceUrl: string): string | null {
		...
		if (
			resourceUrl.startsWith("asset://") ||
			resourceUrl.startsWith("https://asset.localhost/") ||
			resourceUrl.startsWith("http://asset.localhost/")
		) {
			try {
				const url = new URL(resourceUrl);
	...
	private isAbsoluteWindowsPath(videoUrl: string): boolean {
	...
	private async resolveVideoResourceUrl(videoUrl: string): Promise<string> {
		...
		try {
			return await resolveMediaPlaybackUrl(localFilePath);
		} catch (error) {
			console.warn(
				"[StreamingVideoDecoder] Failed to resolve media playback URL, falling back to original URL:",
				error,
			);
			return videoUrl;
		}
