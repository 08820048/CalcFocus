...
export function TutorialHelp() {
	...
	const trimShortcut = formatBinding(shortcuts.addTrim, isMac);
		...
		<Dialog>
			<DialogTrigger asChild>
				...
				>
					...
					<span className="font-medium">{t("trimTutorial.trigger", "How trimming works")}</span>
				</Button>
			</DialogTrigger>
			<DialogContent className="max-w-2xl bg-[#09090b] border-white/10 [&>button]:text-slate-400 [&>button:hover]:text-white">
				<DialogHeader>
					<DialogTitle className="text-xl font-semibold text-slate-200 flex items-center gap-2">
						<Scissors className="w-5 h-5 text-[#ef4444]" />{" "}
						{t("trimTutorial.title", "How Trimming Works")}
					</DialogTitle>
					<DialogDescription className="text-slate-400">
						{t(
							"trimTutorial.description",
				...
				<div className="mt-4 space-y-8">
					...
					<div className="space-y-2">
						<h3 className="text-sm font-medium text-slate-400 uppercase tracking-wider">
							{t("trimTutorial.visualExample", "Visual Example")}
						</h3>
						<div className="relative h-24 bg-[#000] rounded-lg border border-white/10 flex items-center px-4 overflow-hidden select-none">
							...
							>
								<span className="text-[10px] font-bold text-[#ef4444] bg-black/50 px-1 rounded">
									{t("trimTutorial.removed", "REMOVED")}
								</span>
							</div>
							{/* Removed Segment 2 */}
							<div
						...
						<div className="relative h-12 bg-[#000] rounded-lg border border-white/10 flex items-center justify-center gap-1 px-4 select-none">
							...
							>
								<span className="text-[10px] text-white font-medium">
									{t("trimTutorial.part3", "Part 3")}
								</span>
							</div>
							<span className="absolute right-4 text-xs text-slate-400">
								{t("trimTutorial.finalVideo", "Final Video")}
							</span>
						</div>
					</div>
					{/* Steps */}
					<div className="grid grid-cols-2 gap-4">
						<div className="p-3 rounded bg-white/5 border border-white/5">
							<div className="text-[#ef4444] font-bold mb-1">
								{t("trimTutorial.step1Title", "1. Add Trim")}
							</div>
							<p className="text-xs text-slate-400">
								{t("trimTutorial.step1Prefix", "Press")}{" "}
								<kbd className="bg-white/10 px-1 rounded text-slate-300">{trimShortcut}</kbd>{" "}
								{t(
									"trimTutorial.step1Suffix",
						...
						<div className="p-3 rounded bg-white/5 border border-white/5">
							<div className="text-[#ef4444] font-bold mb-1">
								{t("trimTutorial.step2Title", "2. Adjust")}
							</div>
							<p className="text-xs text-slate-400">
								{t(
									"trimTutorial.step2Description",
