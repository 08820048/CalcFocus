...
const CURSOR_FALLBACK_ARROW_HOTSPOT = { x: 0.13, y: 0.08 };
const CURSOR_FALLBACK_ARROW_DATA_URL = `data:image/svg+xml;charset=utf-8,${encodeURIComponent(
	`<svg xmlns="http://www.w3.org/2000/svg" width="160" height="160" viewBox="0 0 160 160">
    <path d="M24 14L116 88L78 96L100 144L82 152L60 104L30 132Z" fill="#000000" fill-opacity="0.28"/>
    <path d="M18 8L110 82L72 90L94 138L76 146L54 98L24 126Z" fill="#FFFFFF" stroke="#111827" stroke-width="8" stroke-linejoin="round"/>
  </svg>`,
)}`;
		...
		cursorAssetsPromise = (async () => {
			...
			const entries = await Promise.all(
				SUPPORTED_CURSOR_KEYS.map(async (key) => {
					...
					if (!assetUrl) {
						if (key === "arrow") {
							return [key, await createFallbackArrowAsset()] as const;
						}
					...
					try {
						...
						if (uploadedAsset) {
							...
							normalizedAnchor = {
								x: clamp((fallbackAnchor.x * trim.width) / rasterized.width, 0, 1),
								y: clamp((fallbackAnchor.y * trim.height) / rasterized.height, 0, 1),
							};
						} else {
							finalUrl = assetUrl;
						...
						return [key, await createLoadedCursorAsset(finalUrl, normalizedAnchor)] as const;
					} catch (error) {
						if (key === "arrow") {
							console.warn("[CursorRenderer] Falling back to generated arrow cursor asset.", error);
							return [key, await createFallbackArrowAsset()] as const;
						}
...
function findLatestInteractionSample(samples: CursorTelemetryPoint[], timeMs: number) {
	...
	const earliestVisibleClickMs = timeMs - CLICK_RING_FADE_MS;
	for (let index = lo; index >= 0; index -= 1) {
		...
		if (sample.timeMs < earliestVisibleClickMs) {
			break;
		}
