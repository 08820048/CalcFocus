...
pub async fn get_screen_recording_permission_status(app: AppHandle) -> Result<String, String> {
    #[cfg(target_os = "macos")]
    {
        ...
        let granted = app_granted || helper_granted;
...
pub async fn request_screen_recording_permission(app: AppHandle) -> Result<bool, String> {
    #[cfg(target_os = "macos")]
    {
        let app_request_granted =
            run_on_main_thread(&app, request_screen_recording_access_main_process).unwrap_or(false);
        ...
        Ok(app_request_granted || app_preflight_granted || helper_granted)
    }
