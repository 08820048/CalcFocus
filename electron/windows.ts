...
function getEditorWindowQuery(): Record<string, string> {
	...
	if (process.env.CALCFOCUS_SMOKE_EXPORT === "1") {
		query.smokeExport = "1";
		if (process.env.CALCFOCUS_SMOKE_EXPORT_INPUT) {
			query.smokeInput = process.env.CALCFOCUS_SMOKE_EXPORT_INPUT;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_OUTPUT) {
			query.smokeOutput = process.env.CALCFOCUS_SMOKE_EXPORT_OUTPUT;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_USE_NATIVE === "1") {
			query.smokeUseNativeExport = "1";
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_ENCODING_MODE) {
			query.smokeEncodingMode = process.env.CALCFOCUS_SMOKE_EXPORT_ENCODING_MODE;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_SHADOW_INTENSITY) {
			query.smokeShadowIntensity = process.env.CALCFOCUS_SMOKE_EXPORT_SHADOW_INTENSITY;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_INPUT) {
			query.smokeWebcamInput = process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_INPUT;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_SHADOW) {
			query.smokeWebcamShadow = process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_SHADOW;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_SIZE) {
			query.smokeWebcamSize = process.env.CALCFOCUS_SMOKE_EXPORT_WEBCAM_SIZE;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_PIPELINE) {
			query.smokePipelineModel = process.env.CALCFOCUS_SMOKE_EXPORT_PIPELINE;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_BACKEND) {
			query.smokeBackendPreference = process.env.CALCFOCUS_SMOKE_EXPORT_BACKEND;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_MAX_ENCODE_QUEUE) {
			query.smokeMaxEncodeQueue = process.env.CALCFOCUS_SMOKE_EXPORT_MAX_ENCODE_QUEUE;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_MAX_DECODE_QUEUE) {
			query.smokeMaxDecodeQueue = process.env.CALCFOCUS_SMOKE_EXPORT_MAX_DECODE_QUEUE;
		}
		if (process.env.CALCFOCUS_SMOKE_EXPORT_MAX_PENDING_FRAMES) {
			query.smokeMaxPendingFrames = process.env.CALCFOCUS_SMOKE_EXPORT_MAX_PENDING_FRAMES;
		}
	}
