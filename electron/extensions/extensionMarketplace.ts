 ...
 * CalcFocus marketplace API. Also provides admin review endpoints.
 */
...
const MARKETPLACE_API_BASE = 'https://marketplace.calcfocus.dev/extensions/api/v1';
...
function getMarketplaceUrl(): string {
  ...
  if (process.env.CALCFOCUS_MARKETPLACE_URL) return process.env.CALCFOCUS_MARKETPLACE_URL;
  return MARKETPLACE_API_BASE;
}

function getAdminKey(): string | undefined {
  return process.env.CALCFOCUS_ADMIN_KEY;
}
...
async function marketplaceFetch<T>(
  ...
  try {
    const headers: Record<string, string> = {
      ...
      'X-CalcFocus-Version': app.getVersion(),
      'X-CalcFocus-Platform': process.platform,
    };

    // Attach admin key for privileged endpoints
    if (options.admin) {
      const key = getAdminKey();
      if (!key) throw new Error('Admin key not configured (set CALCFOCUS_ADMIN_KEY env var)');
...
export async function downloadAndInstallExtension(
  ...
  const allowedOrigins = [
    'https://marketplace.calcfocus.dev',
    'https://calcfocus.dev',
  ...
  const tempDir = path.join(app.getPath('temp'), `calcfocus-ext-${extensionId}-${Date.now()}`);
  ...
  try {
    ...
    if (dirs.length === 1 && !existsSync(path.join(extractDir, 'calcfocus-extension.json'))) {
      manifestDir = path.join(extractDir, dirs[0].name);
    }

    // Verify manifest exists
    if (!existsSync(path.join(manifestDir, 'calcfocus-extension.json'))) {
      throw new Error('Downloaded extension does not contain a calcfocus-extension.json manifest');
    }
    ...
    fetch(`${MARKETPLACE_API_BASE}/extensions/${encodeURIComponent(extensionId)}/download`, {
      method: 'POST',
      headers: { 'X-CalcFocus-Version': app.getVersion() },
    }).catch(() => {});
