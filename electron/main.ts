...
const IS_SMOKE_EXPORT = process.env.CALCFOCUS_SMOKE_EXPORT === "1";
...
function getDefaultTrayIcon() {
	if (!defaultTrayIcon) {
		defaultTrayIcon = getTrayIcon("app-icons/calcfocus-32.png");
	}
...
function syncDockIcon() {
	...
	const dockIcon = getAppImage("app-icons/calcfocus-512.png");
	if (!dockIcon.isEmpty()) {
...
function getUpdateNotificationTitle(payload: UpdateToastPayload) {
	switch (payload.phase) {
		case "available":
			return `CalcFocus ${payload.version} is available`;
		case "downloading":
			return `Downloading CalcFocus ${payload.version}`;
		case "ready":
			return `CalcFocus ${payload.version} is ready`;
		case "error":
			return `CalcFocus ${payload.version} needs attention`;
	}
}
...
function getUpdateNotificationBody(payload: UpdateToastPayload) {
	switch (payload.phase) {
		...
		case "downloading":
			return "CalcFocus is downloading the update in the foreground.";
		case "ready":
...
function sendUpdateToastToWindows(channel: "update-toast-state", payload: unknown) {
	if (process.platform !== "darwin") {
		...
		const notification = new Notification({
			...
			icon: getAppImage("app-icons/calcfocus-128.png"),
			silent: false,
		});
...
function updateTrayMenu(recording: boolean = false) {
	...
	const trayToolTip = recording ? `Recording: ${selectedSourceName}` : "CalcFocus";
...
app.whenReady().then(async () => {
	if (process.platform === "win32") {
		app.setAppUserModelId("dev.calcfocus.app");
	}
	...
	if (IS_SMOKE_EXPORT) {
		...
		console.log(
			`[smoke-export] Starting editor smoke export for ${process.env.CALCFOCUS_SMOKE_EXPORT_INPUT ?? "<missing input>"}`,
		);
		createEditorWindowWrapper();
		return;
	}
