...
const UPDATER_LOG_PATH =
	process.env.CALCFOCUS_UPDATER_LOG_PATH?.trim() || path.join(USER_DATA_PATH, "updater.log");
...
function createAutoCheckErrorToastPayload(): UpdateToastPayload {
	return {
		...
		phase: "error",
		detail: "CalcFocus could not check for updates automatically. Retry now, or inspect updater.log in your user data folder.",
	...
	return {
		...
		detail:
			normalizedProgress >= 100
				? "Finishing the update download. You can keep using CalcFocus while this completes."
...
async function showNoUpdatesDialog(getMainWindow: () => BrowserWindow | null) {
	await showMessageBox(getMainWindow, {
		...
		message: "CalcFocus is up to date.",
...
async function showUpdateErrorDialog(getMainWindow: () => BrowserWindow | null, error: unknown) {
	await showMessageBox(getMainWindow, {
		...
		message: "CalcFocus could not check for updates.",
		detail: String(error),
	});
}
...
export async function downloadAvailableUpdate(sendToRenderer?: UpdateToastSender) {
	...
	setUpdateStatusSummary({
		status: "downloading",
		availableVersion,
		detail: `Downloading CalcFocus ${availableVersion}`,
	});
...
async function showAvailableUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		message: `CalcFocus ${version} is available.`,
...
async function showDownloadedUpdateDialog(
	...
	const result = await showMessageBox(getMainWindow, {
		...
		title: "Update Ready",
		message: isPreview
			? `CalcFocus ${version} is ready to install.`
			: `CalcFocus ${version} has been downloaded.`,
		detail: isPreview
	...
	if (updateCheckInProgress) {
		...
		if (options?.manual) {
			await showMessageBox(getMainWindow, {
				...
				message: "CalcFocus is already checking for updates.",
			});
		}
		return;
	}
...
export function setupAutoUpdates(
	...
	autoUpdater.on("update-available", (info) => {
		...
		setUpdateStatusSummary({
			status: "available",
			availableVersion: info.version,
			detail: `CalcFocus ${info.version} is available.`,
		});
	...
	autoUpdater.on("update-not-available", () => {
		...
		setUpdateStatusSummary({
			status: "up-to-date",
			availableVersion: null,
			detail: `CalcFocus ${app.getVersion()} is up to date.`,
		});
		clearVisibleUpdateToast(sendToRenderer);
		const shouldReport = manualCheckRequested;
		manualCheckRequested = false;
	...
	autoUpdater.on("update-downloaded", (info) => {
		...
		setUpdateStatusSummary({
			status: "ready",
			availableVersion: info.version,
			detail: `CalcFocus ${info.version} is ready to install.`,
		});
		clearDeferredReminderTimer();
